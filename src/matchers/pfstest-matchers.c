                                  ...
                                  pfstest_value_t *actual_value)
{
    ...
    /* There are use cases for comparing two different-sized blocks of
     * memory. For example, a value whose size is returned from the
     * code under test. This should almost always result in a failed
     * match. */
    if (expected_size != actual_size)
    {
        /* However, if actual_size is 0, someone is comparing a memory
         * block with a value created with something like
         * the_pointer(). Mocks generated by automock, for eample, use
         * the_pointer() for all of their arguments. In this case,
         * proceed with a memcmp() and assume that 'actual' points to
         * a block the same size as 'expected'.
         *
         * NB: automock can get away with using the_pointer() for
         * everything because the data pointed to is never printed,
         * and probably can't be reliably since it may live on the
         * stack and be invalid once mock invocation has completed. */
        if (actual_size != 0)
